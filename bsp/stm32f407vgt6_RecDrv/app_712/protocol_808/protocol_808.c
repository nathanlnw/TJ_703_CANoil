...
u8  Stuff_Normal_Data_0200H(void)  
{
 ...
 memcpy(spd_sensorReg,Original_info+Original_info_Wr,2);// 把读取传感器速度 
  ...
  //0                                           ----- 油耗ml  
  if(Oil_Cau.Oil_deltaPacket_stuff_dot1ml) // 判断是否有油耗
{
	  //  附加信息 ID
	  Original_info[Original_info_Wr++]=0xEE; // 油量消耗ml
	  //  附加信息长度
	  Original_info[Original_info_Wr++]=4;
	  //  类型
	  Dis_01km=Oil_Cau.Oil_deltaPacket_stuff_dot1ml/10;// ml  
	  Original_info[Original_info_Wr++]=(u8)(Dis_01km>>24); 
	  Original_info[Original_info_Wr++]=(u8)(Dis_01km>>16); 
	  Original_info[Original_info_Wr++]=(u8)(Dis_01km>>8);  
	  Original_info[Original_info_Wr++]=(u8)(Dis_01km); 
	  
	  
	  Oil_Cau.Day_Oil_consumpate_dot1mL+=Oil_Cau.Oil_deltaPacket_stuff_dot1ml; //  accumulate every packet      
	  Oil_Cau.Oil_deltaPacket_stuff_dot1ml=0; // clear
}
...
{  
 ...
 // 2. content 
	...
	//0 										  ----- 油耗ml	
	if(Oil_Cau.Oil_deltaPacket_stuff_dot1ml)
	{
