...
#ifdef  TRUCK_MIXER
//   搅拌车
#define MixerTaskInterval 1

u8  Mixer_Get_staus=0; // 搅拌车  2根线，两根同事获得新的信息才能判断  0x11 判断  0x00 清除
u16 TruckStopRoundTimer=18000;//180s//搅拌车转动一圈所需的最大时间，即是搅拌车停转所需要的判断时间
u32 Curr_front_GreenLevel=0,Last_front_GreenLevel=0x1000;
u32 Curr_back_GreyLevel=0,Last_back_GreyLevel=0x800;
u32 Green_HighCounter_front=0,Grey_HighCounter_back=0;
u32 MixerStopTimer=0; 
...
void Truck_Mixer(void)
{

        Curr_front_GreenLevel=(!GPIO_ReadInputDataBit(FOGLIGHT_IO_Group,FOGLIGHT_Group_NUM));	// PC3   雾灯  绿线
		Curr_back_GreyLevel=(!GPIO_ReadInputDataBit(DOORLIGHT_IO_Group,DOORLIGHT_Group_NUM));	// PC1  车门  灰线

		if(Curr_front_GreenLevel^Last_front_GreenLevel)
		{
		    MixerStopTimer=0;
		    if(Curr_front_GreenLevel)//low change to high
		    {
		        Green_HighCounter_front=rt_tick_get();
				
				//rt_kprintf("1CurrTime is :%d \r\n",Green_HighCounter_front);  

				Mixer_Get_staus|=0x10;
				
		    }
			Last_front_GreenLevel=Curr_front_GreenLevel;
		}
		else
		if(Curr_back_GreyLevel^Last_back_GreyLevel)
		{
		    MixerStopTimer=0;
		    if(Curr_back_GreyLevel)//low change to high
		    {
		        Grey_HighCounter_back=rt_tick_get();
				
				//rt_kprintf("3CurrTime is :%d\r\n",Grey_HighCounter_back);
				Mixer_Get_staus|=0x01;
		    }
			Last_back_GreyLevel=Curr_back_GreyLevel;
		}
		else
		{
		    MixerStopTimer+=MixerTaskInterval;
			//if(MixerStopTimer>=(TruckStopRoundTimer*1000))
			if(MixerStopTimer>=TruckStopRoundTimer) 
			{
			    MixerStopTimer=0;
			   // rt_kprintf("\r\n电平没变化超时，停转\r\n"); 
				Mixer_Get_staus=0x00; // goto idle
				// 状态				
				 //Position_info_UDP[1] &= ~0x20;		
			}
		}
		if(((Green_HighCounter_front-Grey_HighCounter_back)>0)&&((Green_HighCounter_front-Grey_HighCounter_back)<=6000))//60S 内必须转半圈，否则检测不出来
		{
		    Green_HighCounter_front=Grey_HighCounter_back; 
			if(Mixer_Get_staus==0x11)			
		    {
			    // rt_kprintf("\r\n----------灰前绿后，反转\r\n");
				//  状态
				//Position_info_UDP[1] |= 0x20;
				//Position_info_UDP[1] |= 0x40;		
				Mixer_Get_staus=0x00; // goto idle
			}
		}
		else if(((Grey_HighCounter_back-Green_HighCounter_front)>0)&&((Grey_HighCounter_back-Green_HighCounter_front)<=6000)) 
		{
		    Green_HighCounter_front=Grey_HighCounter_back; 
		    if(Mixer_Get_staus==0x11)			
		    {
